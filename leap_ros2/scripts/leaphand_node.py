...
class LeapNode(Node):
    def __init__(self):
        ...
        self.leap_position_pub = self.create_publisher(JointState, '/leap_position', 10)

        # create timer to publish
        self.pub_timer = self.create_timer(0.05, self.publish_callback)
        ...
        self.dxl_client.set_torque_enabled(self.motors, True, retries=3)
    ...
    def pos_vel_eff_srv(self, request, response):
        ...
        return response
    # Publish the current position of the hand
    def publish_callback(self):
        msg = JointState()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.name = [f'joint_{i}' for i in range(len(self.curr_pos))]
        msg.position = self.dxl_client.read_pos().tolist()
        self.leap_position_pub.publish(msg)
